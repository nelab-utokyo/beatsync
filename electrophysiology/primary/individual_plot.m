...
figure()
set(gcf, 'Units', 'Normalized', 'OuterPosition', [0, 0.04, 0.2, 0.8]);
for k = 1:length(labels)
    subplot(4,1,k)
    data = df.(names(k));
    trg = round(trg2(1)-100/mult(k)):(trg2(end)-1);
    pkup = (21*mult(k)+1):(21*mult(k)+13);
    resp = psth.("f"+labels(k));
    plot((1:size(resp,2)/4)-1500,mean(resp(:,1:size(resp,2)/4)),"Color",[1,0.5,0],"linewidth",2)
    hold on
    trg2 = data.trg2(pkup);
    ylim([0,280])
    xlim([-1500,size(resp,2)/4-1500])
    xticks([-1500,size(resp,2)/4-1500])
    xticklabels([7,1/mult(k)+7])
    title(labels(k)+ " BPM", "fontsize",28)
    trgdif = mean(trg2(2:3:12) - trg2(1:3:12));
    for i = 1:length(trg2)-1
        plot([1,1]*(trg2(i)-trg(1)-1500), [0,300], "-","color",[0.5,0.5,0.65],"linewidth",1.5)
        if rem(i, 3) == 0
            plot([1,1]*(trg2(i)+trgdif-trg(1)-1500), [0,300], "--","color",[0.5,0.5,0.65],"linewidth",1.5)
        end
    end
    ax = gca;
...
saveas(gca,champ+"/figure/PSTH_sample.fig")
saveas(gca,champ+"/figure/PSTH_sample.png")
