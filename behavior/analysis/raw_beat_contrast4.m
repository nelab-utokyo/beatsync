...
figure()
set(gcf, 'Units', 'Normalized', 'OuterPosition', [0, 0.0, 0.8, 0.32]);
   ...
   ax(n) = axes('Position',...
    ...
    for k2 = 1:3
        for k = 1:10
            ...
            %tmp = range(squeeze(mean(indvdata.("no"+k).(mkinds(cond))(:,4,:,k2*2-1:k2*2),4)));
            tmp = squeeze(mean(indvdata.("no"+k).(mkinds(cond))(33,4,:,k2*2-1:k2*2),4));
    ...
    disp([cond,mean(allind,1),p])  %0.5451, 0.032, 0.2137, 0.3957
    X = [ones(1,3); 1:3]';
    Y = X\mean(allind)';
    ...
    if sign >= 1
        for s = 1:sign
            plot(mean(xt([cand(i,1) cand(i,2)]))+(s-sign/2 - 0.5)/4, yt*(0.98+cnt/20) + 0.03, '*k',"markersize",20, 'LineWidth',2)
        end
    else
        text(mean(xt([cand(i,1) cand(i,2)]))-0.3, yt*(0.98+cnt/20)+0.045, "n.s.","Fontsize",30)
    end
...
saveas(gca,path1 + "/summary4/beat_day.fig")
saveas(gca, path1 + "./summary4/beat_day.png")
...
for i = 1:132
    ...
    plot([1,1]*aclick.s100(i)/1000, [-1,10], "k")
    hold on
end
...
if plnum < 4
    ...
    tmp2_ = diff(acc(:,I3:I4),1,2);
    tmp2 = sqrt(tmp2_(1,:).^2+tmp2_(2,:).^2+tmp2_(3,:).^2);
    h = plot(time((I3:(I4-1))-I1),tmp2+1, "linewidth",1.5, "color", "k");
end
