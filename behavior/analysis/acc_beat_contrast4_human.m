...
%saveas(gca, path2 + "summary4/acc_beat_contrast_allr2"+akinds(apkup)+".fig")
%saveas(gca, path2 + "summary4/acc_beat_contrast_allr2"+akinds(apkup)+".png")
...
disp("Kruskalwallis = " + p)  %5.1e-05
disp("friedman = " + friedman(temp,1,"off"))  
result = multcompare(stats, 'CType','bonferroni'); %1-4,2-4 only significant
disp("rank sum = "+ranksum(temp(:,2),temp(:,5))) %0.016575
...
%saveas(gca, path2 + "summary4/range_acc_beat2"+akinds(apkup)+".fig")
%saveas(gca, path2 + "summary4/range_acc_beat2"+akinds(apkup)+".png")
...
%saveas(gca, path2 + "summary4/acc_beat_peakpos_a2"+akinds(apkup)+".fig")
%saveas(gca, path2 + "summary4/acc_beat_peakpos_a2"+akinds(apkup)+".png")
...
disp("anova1 " + anova1(ex, [], "off"))  %1.0e-11

[p,tbl,stats] = kruskalwallis(ex, [], "off");
disp("Kruskalwallis = " + p)  %1.3e-08
disp("friedman = " + friedman(ex,1,"off"))  %4.7e-08
result = multcompare(stats, 'CType','bonferroni');
disp("rank sum = "+ranksum(ex(:,2),ex(:,5))) %9.7346e-05
%
ps = zeros(length(lnames));
for i = 1:size(result,1)
    ps(result(i,1),result(i,2)) = result(i,end);
...
%saveas(gca, path2 + "summary4/acc_beat_peaktest_a2"+akinds(apkup)+".fig")
%saveas(gca, path2 + "summary4/acc_beat_peaktest_a2"+akinds(apkup)+".png")
