...
for k = 1:size(allfiles,2)
    ...
    for j = 1:1
        ...
        for i = 4:4
            for n = 1:size(monoff,1)
                st = monoff(n,2);
                if i < 4
                    tmp2 = abs(diff(acc(i,st-5:monoff(n,3))));
                else
                    tmp2_ = diff(acc(:,st-5:monoff(n,3)),1,2);
    ...
    indvdatab.("no"+k) = monamp;
    indvdatab.("no2"+k) = monamp2;
end
...
chcker = zeros(length(gap_),size(allfiles,2),length(files)*2);
...
for k = 1:size(allfiles,2)
    store = [];
    for knd = 1:4
        tmp = squeeze(indvdatab.("no2"+k).(mkinds(knd))(:,dim,:));
